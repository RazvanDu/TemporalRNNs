...
void forward(int64_t B, int64_t T, int64_t C, torch::Tensor &w, torch::Tensor &u, torch::PackedTensorAccessor32<float,1,torch::RestrictPtrTraits>k, torch::Tensor &v, torch::Tensor &y) {
    cuda_forward(B, T, C, w.data_ptr<float>(), u.data_ptr<float>(), k.accessor<float, 1>(), v.data_ptr<float>(), y.data_ptr<float>());
}
