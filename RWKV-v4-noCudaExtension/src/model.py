...
wkv_cuda = load(name="wkv", sources=["cuda/wkv_op.cpp", "cuda/wkv_cuda.cu"],
                verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}', "-D_GLIBCXX_USE_CXX11_ABI=0"])
        ...
        for i_C in range(C):
           ...
           #print(np.shape(w_C))
            ...
            for i in range(T):
                ...
                k_V = k[i_B][i][i_C]
                #print(np.shape(y))
                #print(np.shape(y[i_B]))
                #print(np.shape(y[i_B][i]))
                #print(np.shape(y[i_B][i][i_C]))

                no = torch.Tensor(max(o, u_C + k_V))

                A = torch.exp(o - no)
                BB = torch.exp(u_C + k_V - no)

                y[i_B][i][i_C] = (A * p + BB * v[i_B][i][i_C]) / (A * q + BB)
