...
class GrebeDataset(Dataset):
    def __init__(self, data, ctx_len, epoch_length_fixed, tokenizer):
    ...
    def __getitem__(self, idx):
        ...
        result = [0 for _ in range(50277)] # TODO WTF self.tokenizer.vocab_size)]

        result[self.data[i+1]] = 1

        x = torch.tensor(self.data[i], dtype=torch.long)
        y = torch.tensor(result, dtype=torch.long)
        return x, y

class GREBE_TOKENIZER():
    ...
    def sample_logits_bef(self, out, x, temperature=1.0, top_p_usual=None, top_p_newline=None):
        ...
        probs = F.softmax(out.clone().detach().cpu(), dim=-1)
...
class Dataset(Dataset):
    def __init__(self, data, ctx_len, epoch_length_fixed, hugging_face=False):
        self.ctx_len = ctx_len
        self.epoch_length_fixed = epoch_length_fixed
        self.data = data
